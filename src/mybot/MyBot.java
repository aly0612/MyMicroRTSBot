...
public class MyBot extends AIWithComputationBudget {
    ...
    void init() {
        ...
        // if enemy have barracks,but don't have combat units, barracksunit for each
        // barracks is ranged
        // for each barracks find closest enemy unit and if it is heavy change
        // barracksunit to ranged
        // for each barracks find closest enemy unit and if it is ranged change
        // barracksunit to light
        // for each barracks find closest enemy unit and if it is light change
        // barracksunit to heavy
    ...
    public static class Pos {
        ...
        public List<Pos> adjacentPos() {
            return Stream.of(
                    ...
                    new Pos(x, y - 1),
                    new Pos(x + 1, y),
                    new Pos(x - 1, y)).collect(Collectors.toList());
        }
    ...
    boolean availablePos(Pos pos) {
        int rasterPos = pos.getX() + pos.getY() * physicalGameState.getWidth();
        if (pos.getX() < 0 || pos.getX() >= physicalGameState.getWidth() || pos.getY() < 0
                || pos.getY() >= physicalGameState.getHeight())
            return false;
    ...
    boolean attackUnitInRange(Unit u) {
        ...
        if (lowestHPEnemy != null) {
            playerAction.addUnitAction(u,
                    new UnitAction(UnitAction.TYPE_ATTACK_LOCATION, lowestHPEnemy.getX(), lowestHPEnemy.getY()));
    ...
    void workerAction() {
        ...
        if (resourcesUsed + barracksType.cost <= player.getResources() && workersAllay.size() > 1
                && freeWorkers.size() > 0) {
            ...
            if (barracksAllay.size() + barracksUnderConstruction.size() < maxBarracks) {
                ...
                if (distance(closestWorker, pos) > 1) {
                    ...
                    playerAction.addUnitAction(closestWorker,
                            new UnitAction(UnitAction.TYPE_PRODUCE, dir, barracksType));
                    freeWorkers.remove(closestWorker);
        ...
        if (resourcesUsed + baseType.cost + 10 <= player.getResources() && workersAllay.size() > 1
                && freeWorkers.size() > 0) {
        ...
        // for each resouce find closet worker and add it to myHarvestWorkers until
        // maxHarvestWorkers
        while (true) {
    ...
    void barracksAction() {
        ...
        // if resource is enough to produce unit, produce it
        for (int i = 0; i < barracksAllay.size(); i++) {
    ...
    private void harrassWithRangedUnits(){
        for(Unit rangedUnit : rangedAllay){
            if(busy(rangedUnit)){
                continue;
            }

            Unit target = findHarassTarget(rangedUnit);
            if (target != null) {
                // Check if the target is within attack range
                if (isInRange(rangedUnit, target)) {
                    // Attack if within range
                    playerAction.addUnitAction(rangedUnit, new UnitAction(UnitAction.TYPE_ATTACK_LOCATION, target.getX(), target.getY()));
                } else {
                    // Move closer to the target if not within attack range
                    int dir = findMoveDir(rangedUnit, target.getX(), target.getY());
                    if (dir != -1) {
                        playerAction.addUnitAction(rangedUnit, new UnitAction(UnitAction.TYPE_MOVE, dir));
                    }
                }
            } else {
                // If no harassment target is found, fallback to a secondary objective or positioning
    ...
    private Unit findHarassTarget(Unit rangedUnit){
        // Find the closest enemy worker
        Unit closestWorker = getClosestUnit(rangedUnit, workersEnemy);
        if (closestWorker != null) {
            return closestWorker;
        }

        // Find the closest enemy ranged unit
        Unit closestRanged = getClosestUnit(rangedUnit, rangedEnemy);
        if (closestRanged != null) {
            return closestRanged;
        }

        // Find the closest enemy base
        Unit closestBase = getClosestUnit(rangedUnit, basesEnemy);
        if (closestBase != null) {
            return closestBase;
        }

        // Find the closest enemy barracks
        Unit closestBarracks = getClosestUnit(rangedUnit, barracksEnemy);
